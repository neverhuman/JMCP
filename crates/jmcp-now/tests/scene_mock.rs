...
use jmcp_domain::{
    AttentionLevel, CardLod, CounterValue, PaneKind, PanePreview, PaneRisk, PaneStatus,
    PreparedTab, WorkOrderStatus,
};
use jmcp_now::{
    queue_blockers_panes, queue_blockers_projection, CachedNow, NowProjection, NowReads,
};
use mock_support::{
    ...
    NowReads {
        ...
        incidents: vec![
            incident_record(
                ...
                "Bridge write lease remains quarantined",
            ),
            incident_record(
                submitted,
                jmcp_domain::IncidentSeverity::Critical,
                "Submitted blocker is high blast radius",
            ),
        ],
...
fn mixed_reads_render_canonical_pane_fields_and_ordered_sidecars() {
    ...
    assert!(focused
        .preview
        .chips
        .contains(&"awaiting_approval".to_owned()));
    ...
    assert_eq!(
        ...
        vec![
            jmcp_domain::PaneCounter {
                label: "evidence".to_owned(),
                value: CounterValue::Number(1)
            },
            jmcp_domain::PaneCounter {
            ...
            jmcp_domain::PaneCounter {
                ...
                value: CounterValue::Text("awaiting_approval".to_owned())
            },
            jmcp_domain::PaneCounter {
                label: "leaseMins".to_owned(),
                value: CounterValue::Number(10)
            },
            jmcp_domain::PaneCounter {
                label: "approvalMins".to_owned(),
                value: CounterValue::Number(20)
            },
        ],
    );
    assert_eq!(
        focused.prepared_tabs,
        vec![
    ...
    let leased = panes
        .iter()
        .find(|pane| pane.title == "Lease-only blocker")
        .expect("leased pane");
    ...
    assert!(leased
        .preview
        .headline
        .contains("Lease held by jmcpd needs completion or renewal."));
    ...
    let submitted = panes
        .iter()
        .find(|pane| pane.title == "Submitted blocker")
        .expect("submitted pane");
    assert_eq!(submitted.status, PaneStatus::Incubating);
    assert_eq!(submitted.lod, CardLod::Preview);
    assert!(submitted
        .preview
        .headline
        .contains("Submitted work order is waiting for a lease or approval path."));
    assert!(submitted.preview.chips.contains(&"bounded_auto".to_owned()));
    assert!(submitted
        .preview
        .chips
        .contains(&"incident_critical".to_owned()));

    let rank_ids = projection
        .rank_reasons
        .iter()
        .map(|reason| reason.pane_id.as_str())
        .collect::<Vec<_>>();
    let pane_ids = panes
        .iter()
...
fn failed_work_order_without_any_automation_path_is_manual_only() {
    ...
    assert_eq!(
        actions[0].safety,
        jmcp_domain::ActionSafetyClass::ManualOnly
    );
}
...
fn now_projection_refreshes_on_watermark_change_and_loads_without_changing_snapshot() {
    ...
    assert_eq!(
        refreshed.default_pane,
        jmcp_now::scenes::queue_blockers::KEY
    );
}
